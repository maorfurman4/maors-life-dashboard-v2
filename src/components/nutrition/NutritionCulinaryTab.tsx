...
⚠️ חובה מוחלט: חשב את ערכי החלבון והקלוריות לפי הטבלה ולפי הכמויות שבחרת. אסור לכתוב את המספרים הנדרשים אם הם לא תואמים את חישוב המרכיבים. אם 2 ביצים = 12g חלבון ואתה צריך 5g — אתה חייב לשנות את הכמות (לא לכתוב 5g בשקר). כתוב בדיוק מה שיוצא מהחישוב.` : "";
      ...
      const allergyLine = localAllergies.length
        ? `מרכיבים אסורים (אלרגיות): ${localAllergies.join(", ")} — אסור לכלול בשום צורה.`
        : "";

      const prompt = `אתה שף ודיאטן ישראלי. צור 3 מתכונים. פלט JSON בלבד — ללא טקסט, ללא הסברים, ללא חישובים גלויים.
${dietRules ? `
חוקי דיאטה (חובה מוחלט — כל מרכיב שאסור בדיאטה מוחלף אוטומטית):
${dietRules}` : ""}
${allergyLine}
${scanDone ? `מרכיבים במקרר בלבד: "${query}"${fridgeConstraint}` : `בקשה: "${query}"`}
${NUTRITION_TABLE}
${macroTargetBlock}

חוקי איכות:
- כמויות תבלינים מדויקות: "½ כפית כמון", "1 כפית פפריקה", "¼ כפית פלפל שחור"
- חלבון וקלוריות: חשב פנימית לפי הטבלה ורשום ערכים מדויקים
- מגוון: 3 מנות שונות

{"recipes":[{"name":"שם","difficulty":"קל","time_minutes":20,"servings":2,"calories_per_serving":350,"protein_per_serving":25,"tags":["בריא"],"ingredients":["200g חזה עוף","1 כף שמן זית","½ כפית כמון"],"steps":["שלב 1...","שלב 2..."],"tip":"טיפ"},{"name":"..."},{"name":"..."}]}

difficulty: קל/בינוני/מתקדם בלבד. JSON תקני בלבד.`;
